package com.fangcs.sms.protocol.smgp.message;

import com.fangcs.sms.protocol.smgp.tlv.TLVByte;
import com.fangcs.sms.protocol.smgp.tlv.TLVString;
import com.fangcs.sms.protocol.smgp.util.ByteUtil;
...
public class SMGPSubmitMessage extends SMGPBaseMessage {
	...
	private TLVByte chargeUserType = new TLVByte(
			SMGPConstants.OPT_CHARGE_USER_TYPE);
	private TLVByte chargeTermType = new TLVByte(
			SMGPConstants.OPT_CHARGE_TERM_TYPE);
	private TLVString chargeTermPseudo = new TLVString(
			SMGPConstants.OPT_CHARGE_TERM_PSEUDO);
	private TLVByte destTermType = new TLVByte(SMGPConstants.OPT_DEST_TERM_TYPE);
	private TLVString destTermPseudo = new TLVString(
			SMGPConstants.OPT_DEST_TERM_PSEUDO);
	...
	private TLVByte submitMsgType = new TLVByte(
			SMGPConstants.OPT_SUBMIT_MSG_TYPE);
	...
	protected int setBody(byte[] bodyBytes) throws Exception {
		...
		if (destTermIdCount >= 100 || destTermIdCount <= 0) {
			throw new Exception("destTermIdCount must be in [1,99],but "
					+ destTermIdCount);
		}
	...
	protected byte[] getBody() throws Exception {
		int len = 0 + 1 + 1 + 1 + 10 + 2 + 6 + 6 + 1 + 17 + 17 + 21 + 21 + 1
				+ 21 * destTermIdCount + 1 + msgLength + 8;
		int offset = 0;
		...
		for (int i = 0; i < destTermIdCount; i++) {
			ByteUtil.rfillBytes(destTermIdArray[i].getBytes(), 21, bodyBytes,
					offset);
			offset += 21;
		}
	...
	public void setDestTermIdArray(String[] destTermIdArray) {
		...
		this.destTermIdCount = (byte) (destTermIdArray == null ? 0
				: destTermIdArray.length);
	}
	...
	public String toString() {
		...
		buffer.append("SMGPSubmitMessage:[sequenceNumber=")
